...
def create_mixed_reward_scorer(
    ...
    bleu_scorer = create_sacrebleu_reward_scorer("bleu")
    ...
    def score(
        ...
        bleu_scores = bleu_scorer(predictions, references, sources)
        comet_scores = comet_scorer(predictions, references, sources)

        return [
            0.25 * (chrf_score / 60.0)
            + 0.15 * (bleu_score / 32.0)
            + 0.60 * (comet_score / 0.845)
            for chrf_score, bleu_score, comet_score in zip(
                chrf_scores, bleu_scores, comet_scores, strict=True
            )
        ]
