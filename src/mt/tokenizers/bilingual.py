...
class BilingualBaseTokenizer:
    ...
    def target_token_id(self, target_token: str) -> int:
        id = self.tokenizer.token_to_id(target_token)
        if id is None:
    ...
    def _create_tokenizer(self) -> Tokenizer:
        ...
        tokenizer.pre_tokenizer = Sequence(
            [Metaspace(), Punctuation(), Digits(individual_digits=True)]
        )
    ...
    def _insert_target_token(self, encoding: Encoding, target_token: str | None) -> Encoding:
        if target_token is None:
            return encoding

        target_encoding = self.tokenizer.encode(target_token, add_special_tokens=False)
        return Encoding.merge([target_encoding, encoding], growing_offsets=False)
    ...
    def encode(
        ...
        return self._insert_target_token(encoding, target_token)
    ...
    def encode_batch(
        ...
        is_pretokenized: bool = False,
        add_special_tokens: bool = True,
        target_tokens: list[str | None] | str | None = None,
    ) -> list[Encoding]:
        ...
        if isinstance(target_tokens, list):
            return [
                self._insert_target_token(encoding, target_token)
                for encoding, target_token in zip(encodings, target_tokens, strict=True)
            ]

        return [self._insert_target_token(encoding, target_tokens) for encoding in encodings]
    ...
    def enable_padding(self, direction: str):
        self.tokenizer.enable_padding(
            direction=direction, pad_id=self.pad_token_id, pad_token=self.pad_token
        )
    ...
    def train_from_iterator(
        self, iterator: Iterator[str], vocab_size: int = 32000, show_progress: bool = True
    ):
        trainer = self._create_trainer(vocab_size=vocab_size, show_progress=show_progress)
    ...
    def train(
        self, files: list[str | PathLike], vocab_size: int = 32000, show_progress: bool = True
    ):
        trainer = self._create_trainer(vocab_size=vocab_size, show_progress=show_progress)
        self.tokenizer = self._create_tokenizer()
